...
class EvalAgentTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.timeout_bouns = int(testDict['timeout_bouns']) if 'timeout_bouns' in testDict else 2
        ...
        self.maxPoints = sum([len(t) for t in [
                             ...
                             self.winsThresholds]]) if 'maxPoints' not in testDict else int(testDict['maxPoints'])
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        def gradeThreshold(value, minimum, thresholds, name, pt=2):
            points = 0
            ...
            if passed:
                for t in thresholds:
                    if value >= t:
                        points += pt
            return (passed, points, value, minimum, thresholds, name, pt)
        ...
        results = [gradeThreshold(averageScore, self.scoreMinimum, self.scoreThresholds, "average score"),
                   gradeThreshold(nonTimeouts, self.nonTimeoutMinimum,
                                  self.nonTimeoutThresholds, "games not timed out", self.timeout_bouns),
                   gradeThreshold(wins, self.winsMinimum, self.winsThresholds, "wins", pt=3)]

        totalPoints = 0
        # st()
        for passed, points, value, minimum, thresholds, name, pt in results:
            ...
            if not passed:
                ...
                self.addMessage("%s %s (%s of %s points)" %
                                (value, name, points, len(thresholds)*pt))
            if minimum != None:
        ...
        if any([not passed for passed, _, _, _, _, _, _ in results]):
            totalPoints = 0
